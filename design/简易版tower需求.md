...
生产部署提供 Docker Compose 编排：`docker-compose.prod.yml` 同时启动 PostgreSQL、Redis、后端 API 和前端 Nginx。前端 Nginx 负责静态资源，并代理 `/api` 与 WebSocket 到后端。首次部署执行 `cp .env.example .env` 后运行 `npm run deploy:up`；全新演示环境如需 seed 可运行 `npm run deploy:up:seed`。默认访问端口由 `WEB_PORT` 控制，默认 `8080`；正式域名部署时必须把 `DEPLOY_APP_BASE_URL` 设置成外部访问地址。`npm run deploy:down` 停止服务并保留数据卷，`npm run deploy:down:volumes` 会删除 PostgreSQL / Redis 数据卷并清空数据。
...
- 是否商业化或闭源；如果调整发布方式，需要重新确认第三方依赖、Docker 镜像和对外服务的许可证与使用条款。
- 是否必须支持移动端拖拽；如果必须，需要单独设计和验证触摸交互方案。
- 是否开放超过两级的多层子任务；当前产品只开放两级子任务，底层模型预留多层能力。
- 是否建设跨项目排期总览；当前甘特图是项目级视图，暂不提供跨项目资源排期。
- 是否引入任务依赖和甘特图依赖线；TODO 已记录第一版建议只做依赖关系维护和可视化，不做自动排程。
