...
- 系统 ADMIN 可在团队设置里设置或取消默认团队；默认团队用于普通用户注册或登录时自动加入。
...
- 系统 ADMIN 或团队 ADMIN 可在工作台打开团队级项目回收站，查看已删除项目、删除人和删除时间，并恢复项目。
...
- 工作台团队区提供“项目回收站”入口，仅系统 ADMIN、当前团队 ADMIN 可见。
...
- 系统 ADMIN 即使不是团队或项目成员，也应收到团队 / 项目管理相关 WebSocket 事件，用于刷新团队设置、项目设置、邀请记录、默认团队 / 默认项目和回收站等管理视图。
