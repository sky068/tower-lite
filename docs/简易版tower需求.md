...
当前甘特图不提供独立数据接口，也不提供独立排期接口；它复用项目列表页的任务树数据，拖拽排期时通过任务更新接口写回 `startDate` 和 `dueDate`。WebSocket 任务和标签变更需要刷新甘特图缓存。任务依赖模型已预留，但依赖 API、循环校验、依赖线 UI 和自动排程尚未实现，属于后续排期增强能力。
