...
V2.0 先落地项目级甘特图只读展示：项目菜单增加“甘特图”入口，读取现有任务开始日期和截止日期生成时间轴；支持关键词、负责人、优先级、完成状态筛选，筛选命中子任务时保留父任务上下文；点击任务名称或时间轴任务条打开任务详情。没有开始日期或截止日期的任务归入“未排期任务”，暂不支持拖拽改期、缩放切换和依赖连线。
...
- 项目甘特图使用任务开始日期和截止日期进行只读展示；筛选规则与看板和项目列表页保持一致，点击任务名称或任务条进入任务详情。
