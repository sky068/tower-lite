...
- 移动到完成类型列表时必须写入 `completedAt` 和 `completedById`；从完成类型列表移出时必须清空完成信息。
...
- 移动任务必须校验目标列表属于同一个项目。
- 移动到完成类型列表时写入 `completedAt` 和 `completedById`。
- 从完成类型列表移出时清空 `completedAt` 和 `completedById`。
...
- 已覆盖：注册登录、创建团队、团队成员加入、团队 MEMBER 不可创建项目、项目 OWNER 创建项目、团队存在项目时不可删除、默认看板列生成、默认列表不可改名、EDITOR 不可管理列表、VIEWER 不可创建任务、任务创建与多人负责人、我的任务、已完成任务返回完成人和完成时间、任务分配通知、V0.1 两级子任务限制、标签创建/查询/绑定/更新/解绑/删除、任务状态移动通知、通知单条已读和全部已读、评论通知、项目归档后拒绝任务和标签写入。
