...
- 项目甘特图使用任务开始日期和截止日期展示排期，默认按天展示，并支持天 / 周 / 月 / 季度缩放；筛选规则与看板和项目列表页保持一致，点击任务名称或任务条进入任务详情；具备任务编辑权限且项目未归档时，可左右拖动任务条中间区域整体调整排期，排期位置按任务条左端所在时间格计算；拖动任务条左边缘单独调整开始日期，拖动右边缘单独调整截止日期。
