...
- 当前开发阶段未接真实邮件服务时，后端把验证 / 重置邮件写入 `EmailOutbox` 开发邮件箱，并允许非生产前端显示开发调试链接；配置 SMTP 后只走正式邮件投递，不在接口响应中暴露 token。生产环境必须接入邮件或企业通知投递。
...
说明：开发阶段用于保存待发送邮件，便于本地测试；普通用户接口不能直接返回 `actionPath`。生产环境应由邮件投递服务读取并发送。
...
- 已覆盖：注册登录、邮箱变更验证确认、密码重置、创建团队、团队删除成功路径、团队成员加入、团队 MEMBER 不可创建项目、项目 ADMIN 创建项目并自动生成默认清单、团队存在项目时不可删除、创建任务默认进入已有默认清单、ADMIN 管理清单、清单删除进入回收站、清单恢复、清单恢复重名冲突、清单彻底删除、EDITOR 不可管理清单、VIEWER 不可创建任务、任务创建与多人负责人、我的任务、已完成任务返回完成人和完成时间、任务分配通知、V0.1 两级子任务限制、标签创建/查询/绑定/更新/解绑/删除、任务状态变更通知、通知单条已读和全部已读、截止前 24 小时站内提醒去重生成、评论通知、任务删除进入回收站、任务恢复、任务彻底删除、项目归档后拒绝任务和标签写入、取消归档后恢复任务写入。
- 集成测试数据使用唯一邮箱后缀创建，测试结束后按依赖顺序清理。
- 后续可继续补充：更细粒度的异常路径和并发冲突场景。
- 飞书通知重试属于 V1 范围，当前集成测试已覆盖飞书投递记录查询和失败投递重试。
- V3 需要补充：添加不存在邮箱为待认领团队成员、添加已存在且已验证邮箱为已绑定团队成员、待认领团队成员加入项目、待认领项目成员可被指派任务、邮箱验证后自动认领团队 / 项目 / 任务、飞书返回邮箱登录可认领、未验证邮箱不能认领、移除待认领成员后不再认领、待认领负责人不生成个人通知、认领后新通知正常发送、邮箱大小写不影响认领、最后一名已绑定团队 / 项目 ADMIN 不允许被移除或降级。
...
- V3 E2E 需要补充：管理员提前添加未注册邮箱、将待认领成员加入项目并分配任务、待认领成员注册但未验证时不可见团队 / 项目 / 我的任务、邮箱验证后进入工作台可看到对应团队 / 项目 / 我的任务、成员列表和负责人选择器正确显示“未注册”和认领后的头像姓名、删除待认领成员后二次验证不再自动加入。
