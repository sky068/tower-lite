...
- 项目菜单提供“回收站”入口，仅项目 OWNER、团队 OWNER、团队 ADMIN 可见。
- 回收站展示当前项目内已删除清单和单独删除的任务。
...
- 已覆盖：注册登录、创建团队、团队删除成功路径、团队成员加入、团队 MEMBER 不可创建项目、项目 OWNER 创建项目并自动生成默认清单、团队存在项目时不可删除、创建任务默认进入已有默认清单、OWNER 管理清单、清单删除进入回收站、清单恢复、清单恢复重名冲突、清单彻底删除、EDITOR 不可管理清单、VIEWER 不可创建任务、任务创建与多人负责人、我的任务、已完成任务返回完成人和完成时间、任务分配通知、V0.1 两级子任务限制、标签创建/查询/绑定/更新/解绑/删除、任务状态变更通知、通知单条已读和全部已读、截止前 24 小时站内提醒去重生成、评论通知、任务删除进入回收站、任务恢复、任务彻底删除、项目归档后拒绝任务和标签写入。
...
- 已覆盖：用户登录、账号资料和自定义头像上传、进入项目看板、新建项目后自动显示默认清单、通过新建任务弹窗创建任务、看板筛选匹配子任务、项目列表页、列表页筛选条、列表页按清单分组和任务树形展示、列表页清单折叠、列表页任务详情背景路由、清单标题加号、清单三点菜单、清单编辑态、删除有任务清单的二次提示、项目回收站入口、恢复已删除清单和清单内任务、指定负责人、打开任务详情弹窗、任务详情字段编辑保存、创建两级子任务、任务详情评论、拖拽任务到其他清单、通过任务状态标记已完成、负责人登录后在工作台看到“我的任务”、我的任务项目筛选、已完成任务信息和分配通知、通知已读、通过 WebSocket 收到评论通知、项目归档只读、EDITOR 看不到添加清单入口、VIEWER 看不到新建任务入口。
