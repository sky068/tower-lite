...
- 已覆盖：注册登录、创建团队、团队成员加入、团队 MEMBER 不可创建项目、项目 OWNER 创建项目、默认看板列生成、默认列表不可改名、EDITOR 不可管理列表、VIEWER 不可创建任务、任务创建与多人负责人、我的任务、任务分配通知、V0 一层子任务限制、任务状态移动通知、评论通知。
- 集成测试数据使用唯一邮箱后缀创建，测试结束后按依赖顺序清理。
- 后续可继续补充：删除任务、删除团队、项目归档只读、标签接口、通知已读、截止提醒队列。
- 飞书通知重试属于 V1 范围，在 V1 接入飞书后补充。

### E2E 测试

- E2E 使用 Playwright，通过 `npm run test:e2e` 运行；该命令自动启动后端和前端，并连接本地 PostgreSQL。
- 已覆盖：用户登录、进入项目看板、默认三列展示、通过新建任务弹窗创建任务、指定负责人、负责人登录后在工作台看到“我的任务”和分配通知、EDITOR 看不到添加列表入口、VIEWER 看不到新建任务入口。
- `npm run test:acceptance` 串联后端集成测试和前端 E2E，用于 V0 自动验收。
- 后续可继续补充：看板拖拽移动任务、任务详情弹窗编辑、子任务创建、评论通知、通知已读、项目归档只读。
