...
- VIEWER 只能查看已授权项目内容，不能创建评论，也不能修改任务、列表、标签或项目设置。
- EDITOR 聚焦任务协作，可以创建、编辑、移动、删除任务和子任务，也可以分配负责人，但不能修改项目基础信息、管理项目成员、管理自定义看板列表或归档/删除项目。
- 项目归档后，OWNER / EDITOR 也不能继续修改任务、列表、标签或评论；归档项目以只读方式展示。
...
- V0 创建或移动子任务时，后端必须校验父任务本身不是子任务；违反时返回 `BUSINESS_RULE_VIOLATION`。
...
- 已覆盖：注册登录、创建团队、团队成员加入、团队 MEMBER 不可创建项目、项目 OWNER 创建项目、团队存在项目时不可删除、默认看板列生成、默认列表不可改名、EDITOR 不可管理列表、VIEWER 不可创建任务、任务创建与多人负责人、我的任务、任务分配通知、V0 一层子任务限制、标签创建/查询/绑定/更新/解绑/删除、任务状态移动通知、通知单条已读和全部已读、评论通知、项目归档后拒绝任务和标签写入。
- 集成测试数据使用唯一邮箱后缀创建，测试结束后按依赖顺序清理。
- 后续可继续补充：任务删除、团队删除成功路径、截止提醒队列。
- 飞书通知重试属于 V1 范围，在 V1 接入飞书后补充。

### E2E 测试

- E2E 使用 Playwright，通过 `npm run test:e2e` 运行；该命令自动启动后端和前端，并连接本地 PostgreSQL。
- 已覆盖：用户登录、进入项目看板、默认三列展示、通过新建任务弹窗创建任务、指定负责人、打开任务详情弹窗、创建一层子任务、任务详情评论、拖拽任务到已完成列、负责人登录后在工作台看到“我的任务”和分配通知、通过 WebSocket 收到评论通知、EDITOR 看不到添加列表入口、VIEWER 看不到新建任务入口。
