...
- 已覆盖：用户登录、账号资料和自定义头像上传、进入项目看板、空项目无清单、通过新建任务弹窗创建任务并自动生成默认清单、看板筛选匹配子任务、项目列表页、列表页筛选条、列表页按清单分组和任务树形展示、列表页清单折叠、列表页任务详情背景路由、清单标题加号、清单三点菜单、清单编辑态、删除有任务清单的二次提示并删除清单内任务、指定负责人、打开任务详情弹窗、任务详情字段编辑保存、创建两级子任务、任务详情评论、拖拽任务到其他清单、通过任务状态标记已完成、负责人登录后在工作台看到“我的任务”、已完成任务信息和分配通知、通知已读、通过 WebSocket 收到评论通知、项目归档只读、EDITOR 看不到添加清单入口、VIEWER 看不到新建任务入口。
