...
- 用户注册或登录成功后，按邮箱大小写不敏感匹配并自动绑定所有待认领团队成员和项目成员。
- 用户在账号设置中修改绑定邮箱后，也会按新邮箱触发一次待认领成员绑定；当前版本不包含邮箱验证码，后续生产化需要补充邮箱所有权验证。
...
- 注册链接不决定成员身份；团队成员和项目成员表是成员身份的唯一事实来源。用户自行注册 / 登录后，系统按邮箱自动认领待认领成员。
...
- 如果待注册邮箱用户未通过注册链接而是自行注册或登录，后端也会先按邮箱认领对应团队 / 项目成员。
...
- 用户账号邮箱全局唯一，并在注册、登录、修改邮箱和飞书邮箱落地时统一规范化为小写；项目成员来自团队成员，因此同一项目内也不会出现同一邮箱重复加入。
- 待认领成员在团队成员列表中显示默认头像、邮箱和“未注册”状态。
- 待认领成员可修改角色，也可被移除。
- 移除待认领团队成员后，该邮箱用户后续注册或登录时不再认领该团队身份。
...
- 普通用户注册、登录或修改邮箱时，后端只认领同邮箱占位成员；如果没有可认领身份，工作台保持空状态，需要管理员通过邮箱添加成员。登录默认进入工作台，不自动跳转到团队或项目；侧边栏按当前用户可访问的团队和项目展示。
...
6. 普通用户注册、登录或修改邮箱时，系统只认领同邮箱的待认领团队 / 项目身份，不会自动加入任何未预加的团队或项目。
...
| PATCH | `/users/me/email` | 更新绑定邮箱；用于飞书未返回邮箱时补充真实邮箱，也用于后续邮箱认领匹配 |
...
V3 起，`POST /teams/:teamId/members` 输入邮箱和角色；邮箱对应账号已存在时创建已绑定成员，不存在时创建待认领成员并返回注册链接。前端 CSV 批量导入复用该接口逐行添加团队成员，不引入独立文件上传接口。成员相关路径统一使用 `memberId`，不再使用 `userId` 作为路径参数。
...
当前甘特图不提供独立数据接口，也不提供独立排期接口；它复用项目列表页的任务树数据，拖拽排期时通过任务更新接口写回 `startDate` 和 `dueDate`。任务依赖模型已预留，但依赖 API、循环校验、依赖线 UI 和自动排程尚未实现，属于后续排期增强能力。
...
- V3 需要补充：添加不存在邮箱为待认领团队成员、添加已存在邮箱为已绑定团队成员、待认领团队成员加入项目、待认领项目成员可被指派任务、注册后自动认领团队 / 项目 / 任务、登录后认领后来添加的待认领身份、移除待认领成员后不再认领、待认领负责人不生成个人通知、认领后新通知正常发送、邮箱大小写不影响认领、最后一名已绑定团队 / 项目 ADMIN 不允许被移除或降级。

### E2E 测试

- E2E 使用 Playwright，通过 `npm run test:e2e` 运行；该命令自动启动后端和前端，并连接本地 PostgreSQL。
- 已覆盖：用户登录、账号资料和自定义头像上传、团队级项目回收站入口、恢复已删除项目、进入项目看板、新建项目后自动显示默认清单、不存在项目的正式状态页、通过新建任务弹窗创建任务、看板筛选匹配子任务、项目列表页、列表页筛选条、列表页按清单分组和任务树形展示、列表页清单折叠、列表页任务详情背景路由、项目甘特图入口、排期展示、未排期子任务空白行展示、甘特图任务树折叠和折叠状态记忆、缩放和任务条可拖动状态、清单标题加号、清单三点菜单、清单编辑态、删除有任务清单的二次提示、项目回收站入口、恢复已删除清单和清单内任务、指定负责人、打开任务详情弹窗、任务详情字段编辑保存、创建两级子任务、任务详情评论、拖拽任务到其他清单、通过任务状态标记已完成、负责人登录后在工作台看到“我的任务”、我的任务团队 / 项目筛选、已完成任务信息和分配通知、通知已读、通过 WebSocket 收到评论通知、项目归档只读、取消归档、EDITOR 看不到添加清单入口、VIEWER 看不到新建任务入口。
- `npm run test:acceptance` 串联后端集成测试和前端 E2E，用于 V0 自动验收。
- 后续可继续补充：更完整的移动端布局回归和复杂拖拽边界。
- V3 E2E 需要补充：管理员提前添加未注册邮箱、将待认领成员加入项目并分配任务、待认领成员注册或登录后进入工作台可看到对应团队 / 项目 / 我的任务、成员列表和负责人选择器正确显示“未注册”和认领后的头像姓名、删除待认领成员后二次登录不再自动加入。
