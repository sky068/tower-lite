...
- 系统管理员可以创建和删除团队，并在创建团队时指定团队管理员邮箱；邮箱账号已存在则直接加入团队为 `ADMIN`，不存在则生成团队 `ADMIN` 邀请。
- 被邀请邮箱的用户即使没有点邀请链接，只要自行注册或登录，后端也会自动接受匹配邮箱的待接受团队 `ADMIN` 邀请，并把该用户设为团队 `ADMIN`。
- 团队必须至少保留一名团队 `ADMIN` 或一条待接受的团队 `ADMIN` 邀请；系统管理员不自动计入团队 `ADMIN`。
- 系统管理员可在团队设置中设置 / 取消默认团队，在项目设置中设置 / 取消默认项目；默认项目会同步把所属团队设为默认团队。
- 系统管理员是平台管理身份，不默认进入业务协作关系；系统管理员创建项目时必须指定一名当前团队成员作为项目 `ADMIN`，自己不会因创建项目自动成为项目成员。
- 普通用户注册或登录时会自动加入默认团队为 `MEMBER`、默认项目为 `EDITOR`；工作台刷新时仍优先保留用户自己的上次选择。
- 团队 ADMIN 可以创建团队邀请、查看团队邀请记录、撤销待接受的团队邀请。
- 项目 ADMIN 或团队 ADMIN / 系统管理员可以创建项目邀请、查看项目邀请记录、撤销待接受的项目邀请。
...
- 没有开始日期或截止日期的任务显示在“未排期任务”区域。
- 具备任务编辑权限且项目未归档时，可左右拖动已排期任务条中间区域整体调整开始 / 截止日期；拖动只允许横向移动，松手时按任务条左端所在时间格作为新开始时间，并保持任务原持续时间不变。
...
- 项目甘特图支持按任务开始日期和截止日期展示排期，默认按天展示，支持天 / 周 / 月 / 季度缩放，时间轴日期使用两位年份，并复用看板、列表的筛选条件；任务列和未排期任务显示负责人头像，超过 15 条时独立滚动；有任务编辑权限时可拖动任务条中间区域整体调整排期，也可拖动左 / 右边缘单独调整开始日期或截止日期
...
- 工作台提供团队级项目回收站，系统管理员 / 团队 ADMIN 可查看已删除项目、删除人和删除时间，并恢复或彻底删除软删除项目；彻底删除需要二次确认，恢复项目遇到同团队同名未删除项目时会提示冲突，不自动改名
- 项目回收站支持查看、恢复和彻底删除已删除任务与清单；恢复清单遇到同名未删除清单时会提示冲突，不自动改名
- 项目归档后可以在项目设置中取消归档，恢复正常协作
- 创建标签并给任务添加或移除标签
- 管理团队成员
- 团队 ADMIN 可以创建团队邀请，复制邀请链接给指定邮箱用户；邀请接受前可以撤销，接受邀请时要求当前登录邮箱和邀请邮箱一致
- 项目 ADMIN、团队 ADMIN 或系统管理员可以创建项目邀请，接受后会同时加入团队和项目；V0.2 仅生成站内邀请链接，不发送邮件
- 团队 ADMIN 或系统管理员可以在团队设置查看审计日志；项目 ADMIN、团队 ADMIN 或系统管理员可以在项目设置查看审计日志
