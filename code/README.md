...
3. 使用系统管理员账号登录工作台，并先在右上角账号设置里完成邮箱验证。
...
- CSV 批量导入由前端解析 CSV 后提交到后端批量导入接口，后端统一校验重复邮箱，并在单个数据库事务内写入成员、注册链接和审计日志；校验失败时不会产生部分导入成功的数据。
- 未验证邮箱打开注册链接时，前端展示正式引导页，提示先完成邮箱验证，并提供返回工作台或打开账号设置的入口；不会展示后端错误 code/requestId。
...
- 未配置真实邮件服务时，后端会把验证 / 重置邮件写入 `EmailOutbox` 开发邮件箱，并在非生产前端显示开发调试链接；配置 SMTP 后会立即投递邮件，投递成功写入 `sentAt`，投递失败写入 `lastError` 并记录后端日志，不在接口响应中暴露 token。
- 邮箱验证、邮箱变更和密码重置链接统一使用 `.env` 的 `APP_BASE_URL` 生成；本地开发也需要用同一个地址访问前端，避免 `localhost` 和 `127.0.0.1` 混用导致登录态和跨 tab 刷新不共享。
- 所有账号 token 只保存哈希值，带过期时间和 `usedAt`，重复生成同类型 token 会废弃旧 token。
- 注册、登录、发送邮箱验证、密码重置请求、邮箱变更和邮箱变更重发都有限流；请求过快会返回 `RATE_LIMITED / 429`。
- 系统管理员可从右上角头像菜单打开“邮件投递”，查看验证 / 重置邮件的发送状态和失败原因，并在 SMTP 已配置时重试未发送成功的邮件；界面和接口都不会暴露验证链接 token。
...
- 飞书投递和截止提醒使用 BullMQ + Redis 重复任务调度，服务启动时注册队列 worker：截止提醒扫描未来 24 小时内到期任务并生成站内提醒，飞书投递扫描 `FEISHU/PENDING` 或可重试失败记录并按次数重试。
...
- 项目菜单增加“甘特图(任务)”和“甘特图(人员)”入口。
- 甘特图(任务)读取任务开始日期和截止日期生成时间轴，点击任务名称或任务条打开任务详情。
- 甘特图(人员)按项目成员分组展示负责人排期，左侧为人员分组和其负责的自身已排期任务；人员行显示该成员所有已排期任务去重合并后的只读汇总时间段，渲染时会按当前缩放粒度重新归一化并合并，避免周 / 月 / 季度视图中同一时间格内多段重叠；多负责人任务会出现在每个负责人分组下，未分配但已排期的任务进入“未分配”分组；人员视图筛选只显示直接匹配任务，不补父级上下文；同一成员下同时包含有排期的父子任务时，按父子关系缩进展示为局部任务树，人员分组和任务树节点都可以折叠；子任务标题下不展开父级路径，父任务跳转通过任务详情顶部父任务路径完成。
...
- 当前不支持任务依赖连线。
- 当前两种甘特图不提供独立后端接口，复用 `GET /projects/:projectId/tasks` 获取项目任务树，并复用 `GET /projects/:projectId/members` 获取人员分组；甘特图(人员)只展示分配给该成员且任务自身有起止日期的任务，并在成员内部按命中的父子任务关系生成局部任务树；拖拽排期复用 `PATCH /tasks/:taskId` 更新 `startDate` / `dueDate`。任务依赖模型已预留，但依赖 API、循环校验、依赖线 UI 和自动排程尚未实现。
- WebSocket 收到 `task.changed` 或 `tags.changed` 时需要刷新看板、项目任务列表和甘特图。当前甘特图复用 `project-task-list` queryKey；代码中同时预留 `project-gantt` queryKey 的失效规则，未来拆独立甘特图缓存时不能漏掉。

## 导航信息架构

- 左侧边栏作为应用主导航，包含工作台、团队、项目三类入口。
- 工作台只承载个人视角内容，例如我的任务和通知入口，不再承载完整团队 / 项目管理列表；我的任务支持团队、项目、状态和关键词筛选。
- 团队入口下展示当前用户可见的所有团队；邮箱已验证的系统管理员额外显示创建团队入口。
...
- 注册和账号设置支持邮箱验证；修改邮箱只生成验证链接，用户点开验证链接后才真正更新邮箱并触发待认领成员合并；待验证邮箱由后端返回，待验证期间不能再次改为其他邮箱，账号设置可重新发送验证邮件或取消邮箱变更。
- 右上角头像菜单展示用户名字和邮箱，并提供账号设置和退出登录
- 系统管理员右上角头像菜单额外提供“邮件投递”，用于查看验证 / 重置邮件状态、失败原因和重试未发送成功的邮件
...
- 项目甘特图(任务)支持按任务开始日期和截止日期展示排期，默认按天展示，支持天 / 周 / 月 / 季度缩放，时间轴日期使用两位年份，并复用看板、列表的筛选条件；甘特图按项目级任务树展示并支持折叠记忆，筛选时默认展开且仍可临时折叠；完整自身排期任务条为绿色，父任务无真实日期但子孙任务有真实排期时显示红色汇总条，已完成任务条为灰色；未排期任务不继承父任务日期且不显示时间条；任务列和未排期任务显示负责人头像，超过 15 条时独立滚动；有任务编辑权限时可拖动完整排期任务条中间区域整体调整排期，也可拖动左 / 右边缘单独调整开始日期或截止日期
- 项目甘特图(人员)按项目成员分组展示负责人排期，人员行显示该成员所有已排期任务去重合并后的只读汇总时间段，渲染时会按当前缩放粒度重新归一化并合并，避免周 / 月 / 季度视图中同一时间格内多段重叠；成员分组和任务树节点可折叠并记忆折叠状态；多负责人任务在每个负责人分组下各展示一份，拖动任意一处都会更新同一个任务；未分配但已排期的任务进入“未分配”分组；人员视图筛选只显示直接匹配任务，不补父级上下文；人员视图不单独展示“未排期任务”分组；同一成员下同时包含有排期的父子任务时，按父子关系缩进展示为局部任务树；子任务只显示“子任务”标记，打开任务详情后可通过顶部父任务路径跳转查看父任务
