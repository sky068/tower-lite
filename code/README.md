...
6. 普通用户注册、登录或修改邮箱时，系统只会认领同邮箱的待认领成员；不会因为系统配置或项目存在而自动加入团队或项目。
...
- 待注册邮箱用户即使没有点注册链接，只要自行注册或登录，后端也会先按邮箱认领对应团队 / 项目成员。
...
- 普通用户注册、登录或修改邮箱时，后端只认领同邮箱的待认领团队 / 项目身份；如果没有可认领身份，工作台保持空状态，需要管理员通过邮箱添加成员。
...
- 创建团队成员时可设置团队角色 `ADMIN` 或 `MEMBER`；创建项目成员时可设置项目角色 `ADMIN`、`EDITOR` 或 `VIEWER`。待认领成员可以先加入项目并被指派任务，用户后续用相同邮箱注册、登录或在账号设置里改成该邮箱后，会自动认领团队、项目和任务负责人身份。
...
- 注册链接需要登录后接受；如果未登录，会先进入登录页，登录或注册成功后继续认领成员身份。
