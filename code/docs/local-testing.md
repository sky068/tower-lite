...
当前 E2E 会覆盖：登录、右上角头像菜单、账号资料、自定义头像上传、密码修改、项目看板、空项目无清单、新建任务时自动生成默认清单、新建任务弹窗、清单标题加号、清单三点菜单、清单编辑态、删除有任务清单的二次提示并删除清单内任务、背景路由任务详情弹窗、任务详情字段编辑保存、直接任务链接、两级子任务、评论、拖拽任务到其他清单、通过任务状态标记已完成、我的任务和已完成任务信息、分配通知、通知已读、WebSocket 评论通知、项目归档只读，以及 EDITOR / VIEWER 的入口权限。
