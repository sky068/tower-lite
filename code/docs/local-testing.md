...
后端集成测试额外覆盖邮箱预加成员和认领规则：添加不存在邮箱为待认领团队成员、添加已存在且已验证邮箱为已绑定团队成员、待认领团队成员加入项目、待认领项目成员可被指派任务、邮箱变更验证后认领团队 / 项目 / 任务、未验证邮箱注册和登录都不能认领、移除待认领成员后不再认领、待认领负责人认领前不可见我的任务、认领后可见我的任务且不补发历史分配通知、邮箱大小写不影响成员匹配、最后一名已绑定团队 / 项目 ADMIN 不允许被移除或降级。集成测试也覆盖账号限流、系统管理员邮件投递记录查看、非系统管理员和未验证系统管理员禁止查看邮件投递记录、未验证系统管理员不能创建团队、邮件投递列表不暴露 token，以及团队重名、同团队项目重名的拒绝规则。注册链接只作为注册 / 登录入口，成员身份以团队成员和项目成员表为准。
