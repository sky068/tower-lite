...
export function getApiErrorMessage(error: unknown, fallback = "操作失败，请稍后再试") {
  if (axios.isAxiosError(error)) {
    ...
    if (message === "Team must keep at least one owner") {
      return "团队至少需要保留一名管理员或一条待接受的管理员邀请。";
    }
...
export const projectApi = {
  ...
  create(teamId: string, input: { name: string; description?: string; projectAdminUserId?: string }) {
  ...
  addMember(projectId: string, input: { userId: string; role: "ADMIN" | "EDITOR" | "VIEWER" }) {
    return unwrap<Member>(api.post(`/projects/${projectId}/members`, input));
  },
  updateMemberRole(projectId: string, userId: string, role: "ADMIN" | "EDITOR" | "VIEWER") {
...
export const systemApi = {
  defaults() {
    return unwrap<{ defaultTeamId: string | null; defaultProjectId: string | null }>(
      api.get("/system/defaults")
    );
  },
  setDefaultTeam(teamId: string) {
    return unwrap<{ defaultTeamId: string | null; defaultProjectId: string | null }>(
      api.patch(`/teams/${teamId}/default`)
    );
  },
  clearDefaultTeam(teamId: string) {
    return unwrap<{ defaultTeamId: string | null; defaultProjectId: string | null }>(
      api.delete(`/teams/${teamId}/default`)
    );
  },
  setDefaultProject(projectId: string) {
    return unwrap<{ defaultTeamId: string | null; defaultProjectId: string | null }>(
      api.patch(`/projects/${projectId}/default`)
    );
  },
  clearDefaultProject(projectId: string) {
    return unwrap<{ defaultTeamId: string | null; defaultProjectId: string | null }>(
      api.delete(`/projects/${projectId}/default`)
    );
  }
};
